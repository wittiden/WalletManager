...
class TransactionBase(MixinId):
    ...
    def __repr__(self) -> str:
        return f'#{self.item_id} -> {self._operation_type.value}\nStatus: {self._operation_status.value if self._operation_status is not None else self._operation_status}, from_address: {self._from_address}, to_address: {self._to_address}, amount: {self._amount}, fee: {self._fee}, completed_at: {self._completed_at}'
...
class DepositTransaction(TransactionBase):
    ...
    def __post_init__(self) -> None:
        ...
        DomainInvariant.no_empty('deposit_currency', self._currency)
    ...
    def currency(self, value: str) -> None:
        self._currency = DomainInvariant.no_empty('deposit_currency', value)

    def __repr__(self) -> str:
        return f'{super().__repr__()}, currency: {self._currency}'
...
class WithdrawTransaction(TransactionBase):
    ...
    def __post_init__(self) -> None:
        ...
        DomainInvariant.no_empty('withdraw_currency', self._currency)
    ...
    @currency.setter
    def currency(self, value: str) -> None:
        self._currency = DomainInvariant.no_empty('withdraw_currency', value)
...
class ExchangeTransaction(TransactionBase):
    ...
    def __post_init__(self) -> None:
        ...
        DomainInvariant.no_negative('rate', self._rate)
    ...
    def rate(self, value: Decimal) -> None:
        self._rate = DomainInvariant.no_negative('rate', value)

    def __repr__(self) -> str:
        return f'{super().__repr__()}, from_currency: {self._from_currency}, to_currency: {self._to_currency}, rate: {self._rate}'
